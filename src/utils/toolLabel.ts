...
/**
 * Returns a comma-separated list of formatted tool labels for the last N tool calls in a session (for tree description).
 */
export function formatLastToolLabels(session: { messages: { toolCalls: ToolCall[] }[] }, max: number = 2): string {
    const labels: string[] = [];
    for (let i = session.messages.length - 1; i >= 0 && labels.length < max; i--) {
        for (let j = session.messages[i].toolCalls.length - 1; j >= 0 && labels.length < max; j--) {
            labels.unshift(formatToolLabel(session.messages[i].toolCalls[j]));
        }
    }
